...
public class Signin extends CommonData {
	...
	public String[] start(Context ctx, boolean isAutoSign, String cfg, String user, String pwd) {
		...
		try{
			...
			//访问登录页面
			res = Jsoup.connect(loginPageUrl).userAgent(UA_CHROME).timeout(TIME_OUT).referrer(loginPageUrl).ignoreContentType(true).method(Method.GET).execute();
			//保存Cookies
			cookies.putAll(res.cookies());
			
			doc = res.parse();
			String action = doc.select("#form1").attr("action");
			if(action.startsWith("http")) {
				loginSubmitUrl = action;
			} else {
				loginSubmitUrl = "http://sersh.passport.189.cn/Logon/UDBCommon/S/" + action;
			}
			...
			String txtUserID = user;
			String txtPwd = pwd;
			String ValidateCode = "";
			String Readed = "on";
			String ibtn_Login = "";
			String HiddenReg = "http://passport.189.cn/Reg.aspx";
			String HiddenErrMsg = "";
			String TimeMsg = "";
			String pageguid = doc.select("#pageguid").val();
			String hid_ql_flag = "";
			String hid_ql_js_account = "";
			String hid_ql_js_udbtoken = "";
			String hid_ql_js_state = "";
			
			HashMap<String, String> postDatas = new HashMap<String, String>();
			postDatas.put("__EVENTTARGET", __EVENTTARGET);
			postDatas.put("__EVENTARGUMENT", __EVENTARGUMENT);
			postDatas.put("__VIEWSTATE", __VIEWSTATE);
			postDatas.put("__EVENTVALIDATION", __EVENTVALIDATION);
			postDatas.put("txtUserID", txtUserID);
			postDatas.put("txtPwd", txtPwd);
			postDatas.put("ValidateCode", ValidateCode);
			postDatas.put("Readed", Readed);
			postDatas.put("ibtn_Login", ibtn_Login);
			postDatas.put("HiddenReg", HiddenReg);
			postDatas.put("HiddenErrMsg", HiddenErrMsg);
			postDatas.put("TimeMsg", TimeMsg);
			postDatas.put("pageguid", pageguid);
			postDatas.put("hid_ql_flag", hid_ql_flag);
			postDatas.put("hid_ql_js_account", hid_ql_js_account);
			postDatas.put("hid_ql_js_udbtoken", hid_ql_js_udbtoken);
			postDatas.put("hid_ql_js_state", hid_ql_js_state);
			
			//提交登录信息
			res = Jsoup.connect(loginSubmitUrl).data(postDatas).cookies(cookies).userAgent(UA_CHROME).timeout(TIME_OUT).referrer(loginPageUrl).ignoreContentType(true).method(Method.POST).execute();
			cookies.putAll(res.cookies());
